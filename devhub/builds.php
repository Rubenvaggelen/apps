...
// Simpler tag command: avoids shell parsing problems with for-each-ref's %(...) format.
...
<div class="grid">
  ...
  <div class="card"><h2>Laatste Android build</h2><?php if($latestTag===''): ?><div class="big warn">Nog geen build-tag gevonden</div><?php else: ?><div class="big"><?=htmlspecialchars($latestTag)?></div><p class="<?= $buildReady?'ok':'warn' ?>"><?= $buildReady?'✓ Laatste commit heeft een afgeronde release':'● Nieuwere commit/build aanwezig; dit is de laatste afgeronde release' ?></p><div class="actions"><a class="btn ok" href="<?=htmlspecialchars($appDownload)?>">⬇ The One APK</a><a class="btn ok" href="<?=htmlspecialchars($carDownload)?>">⬇ The One Car APK</a></div><?php endif; ?></div>
  <div class="card"><h2>Nieuwe build starten</h2><p class="muted">Dit maakt een lege commit op <strong>main</strong> en pusht die naar GitHub. De bestaande workflow bouwt daarna automatisch The One en The One Car.</p><form method="post" onsubmit="this.querySelector('button').disabled=true;this.querySelector('button').textContent='Build starten…';"><input type="hidden" name="csrf" value="<?=htmlspecialchars($_SESSION['csrf'])?>"><input type="hidden" name="action" value="build"><button class="btn primary" type="submit">▶ Start nieuwe Android build</button></form></div>
