...
object MovieLookup {
    ...
    fun fetchNowPlaying(callback: (List<NowPlayingMovie>, String?) -> Unit) {
        ...
        Thread {
            try {
                val today = java.text.SimpleDateFormat("yyyy-MM-dd", java.util.Locale.US).format(java.util.Date())
                val fourWeeksAgo = java.text.SimpleDateFormat("yyyy-MM-dd", java.util.Locale.US).format(
                    java.util.Date(System.currentTimeMillis() - 28L * 24 * 60 * 60 * 1000)
                )
                // "discover" met een strak datumvenster (laatste 4 weken t/m vandaag) geeft een
                // veel actueler beeld van wat daadwerkelijk nog in de bioscoop draait dan het
                // bredere "now_playing"-endpoint, dat films soms weken te lang laat staan.
                val url = URL(
                    "https://api.themoviedb.org/3/discover/movie" +
                        "?api_key=$apiKey&language=nl-NL&region=NL" +
                        "&with_release_type=2|3" +
                        "&release_date.gte=$fourWeeksAgo&release_date.lte=$today" +
                ...
                for (i in 0 until results.length()) {
                    ...
                    if (title.isNotBlank()) movies.add(NowPlayingMovie(title))
                }
