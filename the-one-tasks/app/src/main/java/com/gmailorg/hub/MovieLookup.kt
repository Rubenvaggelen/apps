...
object MovieLookup {
     ...
     * We filteren op tekstuele gelijkenis met de zoekopdracht (zodat een
     * onverwant resultaat er niet zomaar tussen glipt), maar houden daarbinnen
     * TMDB's eigen relevantie-volgorde aan — die weegt zelf al bekendheid mee,
     * en eigen her-sortering (bv. op populariteit) trekt soms juist de
     * verkeerde, minder bekende titel naar boven wanneer er meerdere exacte
     * tekstmatches bestaan.
     */
    private fun searchTitle(query: String, apiKey: String): TitleHit? {
        ...
        // Tier 0: titel komt (genormaliseerd) exact overeen — pak de eerste
        // in TMDB's eigen volgorde (niet zelf op populariteit her-sorteren).
        hits.firstOrNull { normalize(it.title) == normalizedQuery }?.let { return it }
