...
object MovieLookup {
    ...
    data class NowPlayingMovie(val title: String)

    /** Haalt films op die op dit moment in de Nederlandse bioscoop draaien (TMDB). */
    fun fetchNowPlaying(callback: (List<NowPlayingMovie>, String?) -> Unit) {
        val apiKey = BuildConfig.TMDB_API_KEY
        if (apiKey.isBlank()) {
            callback(emptyList(), "Geen TMDB API key ingesteld.")
            return
        ...
        Thread {
            try {
                val today = java.text.SimpleDateFormat("yyyy-MM-dd", java.util.Locale.US).format(java.util.Date())
                val eightWeeksAgo = java.text.SimpleDateFormat("yyyy-MM-dd", java.util.Locale.US).format(
                    java.util.Date(System.currentTimeMillis() - 56L * 24 * 60 * 60 * 1000)
                )
                // "discover" met een breder datumvenster (laatste 8 weken t/m vandaag) —
                // blockbusters blijven vaak 6+ weken draaien, dus 4 weken was te kort en
                // liet bijv. lang lopende films (zoals een Spider-Man-release) er soms uit.
                // We halen ook 2 pagina's op zodat een populaire titel niet buiten de
                // eerste 20 resultaten valt.
                val movies = mutableListOf<NowPlayingMovie>()
                for (page in 1..2) {
                    val url = URL(
                        "https://api.themoviedb.org/3/discover/movie" +
                            "?api_key=$apiKey&language=nl-NL&region=NL" +
                            "&with_release_type=2|3" +
                            "&release_date.gte=$eightWeeksAgo&release_date.lte=$today" +
                    ...
                    for (i in 0 until results.length()) {
                        ...
                        if (title.isNotBlank() && movies.none { it.title == title }) {
                            movies.add(NowPlayingMovie(title))
                        }
                    }
                }
                mainHandler.post { callback(movies, null) }
            } catch (e: Exception) {
                Log.e(TAG, "Nu in de bioscoop ophalen mislukt", e)
                mainHandler.post { callback(emptyList(), "Ophalen mislukt: ${e.message ?: "onbekende fout"}") }
            }
        }.start()
    }
