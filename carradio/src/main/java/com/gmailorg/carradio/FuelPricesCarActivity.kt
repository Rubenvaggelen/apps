...
class FuelPricesCarActivity : AppCompatActivity() {
    companion object {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.activity_fuel_prices_car)
    ...
    private fun loadPrices() {
        if (!hasLocationPermission()) {
            status.text = "Locatietoegang nodig om benzinepompen binnen 5 km te zoeken."
    ...
    private fun renderStations(stations: List<FuelPriceClient.Station>) {
        ...
        status.text = "${displayed.size}${if (stations.size > MAX_RESULTS) "+" else ""} stations • goedkoopste eerst • opgehaald om $time"
    ...
    private fun createStationCard(index: Int, station: FuelPriceClient.Station): View {
        val card = LinearLayout(this).apply {
            ...
            isClickable = true
            isFocusable = true
            setOnClickListener { chooseNavigation(station) }
        }
    ...
    private fun chooseNavigation(station: FuelPriceClient.Station) {
        val options = arrayOf("Google Maps", "Waze")
        android.app.AlertDialog.Builder(this)
            .setTitle("Navigeer naar ${station.name}")
            .setItems(options) { _, which ->
                if (which == 0) openGoogleMaps(station) else openWaze(station)
            }
            .setNegativeButton("Annuleren", null)
            .show()
    }

    private fun openGoogleMaps(station: FuelPriceClient.Station) {
        val uri = Uri.parse("https://www.google.com/maps/dir/?api=1&destination=${station.lat},${station.lon}&travelmode=driving")
    ...
    private fun openWaze(station: FuelPriceClient.Station) {
    ...
    override fun onRequestPermissionsResult(requestCode: Int, permissions: Array<out String>, grantResults: IntArray) {
        ...
        if (requestCode == LOCATION_REQUEST) {
            if (grantResults.any { it == PackageManager.PERMISSION_GRANTED }) {
                ...
                Toast.makeText(this, "Zonder locatietoegang kan The One Car geen benzinepompen in de buurt zoeken.", Toast.LENGTH_LONG).show()
            }
        }
    }
